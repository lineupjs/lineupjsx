...
class Builder3 extends React.PureComponent<unknown, IBuilder3State> {
    ...
    return (
      <div>
        <LineUp
          data={this.state.arr}
          sidePanel
          highlight={this.state.highlight >= 0 ? this.state.highlight : null}
          sidePanelCollapsed={this.state.sidePanelCollapsed}
        ...
        <div>
          ...
          <button onClick={() => this.setState({ groupBy: this.state.groupBy === 'cat' ? 'cat2' : 'cat' })}>
            Group
          </button>
          <button onClick={() => this.setState({ highlight: this.state.highlight + 20 })}>Highlight</button>
          {this.state.selection.map((d) => (
            <div key={d}>{d}</div>
          ))}
        </div>
      </div>
